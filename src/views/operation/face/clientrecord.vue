<template>
  <div class="app-container">
    <div class="clientpage">
      <el-row type="flex" class="row-bg" justify="end">
        <el-input
          v-model=" listQuery.deviceNo"
          style="max-width: 200px;"
          placeholder="根据设备编号查询..."/>
        <el-button type="primary" style="margin-left:10px;" @click="search">搜索</el-button>
        <el-button @click="reset">重置</el-button>
      </el-row>
    </div>

    <div class="faceData">
      <el-table
        v-loading="listLoading"
        :data="list"
        element-loading-text="Loading"
        border
        fit
        highlight-current-row>
        <el-table-column label="序号" type="index" width="50" fixed align="center">
          <template slot-scope="scope">
            <!-- {{ listQuery.page * listQuery.limit + scope.$index + 1 }} -->
            <span>{{ (listQuery.page-1) * listQuery.limit + scope.$index + 1 }}</span>
          </template>
        </el-table-column>
        <template v-for="item in colTable">
          <el-table-column :fixed="item.fixed" :key="item.displayName" :prop="item.prop" :label="item.label" :min-width="item.width" align="center">
            <template slot-scope="scope">
              <template v-if="item.prop==='imagePath'">
                <img :src="scope.row[item.prop]" style="width:80px;" alt="">
              </template>
              <template v-else>
                <span>{{ scope.row[item.prop] | showstr(item.prop) }}</span>
              </template>
            </template>
          </el-table-column>
        </template>

      </el-table>

      <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="$nextTick(()=>{getList()})" />

    </div>
  </div>
</template>
<style scoped>

</style>

<script>
import { clientRecordList as fetchList } from '@/api/face'
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination'

export default {
  filters: {
    showstr(v, prop) {
      if (prop === 'createTime' || prop === 'endTime') { return parseTime(v) } else if (prop === 'imagePath') { return "<img src='" + v + "'>" }
      return v
    }
  },
  components: {
    Pagination
  },
  data() {
    return {
      colTable: [
        { prop: 'id', label: 'id', fixed: true, width: '' },
        { prop: 'storeNo', label: '门店', fixed: true, width: '' },
        { prop: 'deviceName', label: '设备名称', fixed: true, width: '' },
        { prop: 'deviceNo', label: '设备编号', fixed: true, width: '' },
        { prop: 'imagePath', label: '图片', fixed: true, width: '' },
        { prop: 'status', label: '状态', fixed: true, width: '' },
        { prop: 'faceToken', label: 'faceToken', fixed: true, width: '' },
        { prop: 'createTime', label: '时间', fixed: true, width: '' }
      ],

      listQuery: {
        page: 1,
        limit: 10,
        deviceName: '',
        faceToken: '',
        storeNo: '0002',
        deviceNo: '',
        sort: '+id'
      },

      total: 0,
      list: [],
      formdata: {},
      dialogStatus: '',
      listLoading: false,
      isShowDialog: false,
      optsStatus: [],
      deviceNo: '',

      proporg: {
        label: 'organizationName',
        value: 'organizationId',
        children: 'children'
      },

      showGrantDialog: false,
      defaultGrantTab: 'tabGrantData',
      pages: ''
    }
  },
  computed: {

  },
  created() {
    this.getList()
  },
  methods: {
    reset() { // 重置
      this.listQuery.deviceNo = ''
      this.getList()
    },
    search() { // 搜索
      this.listQuery.page = 1
      this.getList()
    },
    ToView(token) {
      console.log(token)
      this.$router.push({ name: 'operationfacefacesbyfaceset', query: { deviceNo: token }})
    },
    getList() {
      fetchList(this.listQuery).then(res => {
        this.listLoading = false
        this.list = res.data.list || []
        this.total = res.data.total
      })
    }
  }
}
</script>

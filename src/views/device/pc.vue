  ...
  <div >
    ...
    <div class="pcStatics">
      <el-table
        ...
        fit
        border>
        <template v-for="item in pcStaticsTable">
    ...
    <div class="btnsupertable">
      <el-button v-if="deviceauthority.includes('devicegateindexPCCreate')" type="primary" @click="handleCreate" >添加</el-button>
    </div>
        ...
        <el-table-column align="center" prop="created_at" label="操作" width="300">
          <template slot-scope="scope">
            <el-row class="btnrows" >
              <el-button v-if="deviceauthority.includes('devicegateindexPCUpdate')" type="primary" size="small" icon="el-icon-edit" @click="handleEdit(scope.row)">编辑</el-button>
              <el-button v-if="deviceauthority.includes('devicegateindexPCDelete')" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row)">删除</el-button>
              <el-button v-if="deviceauthority.includes('devicegateindexPCConfigParam')" type="primary" size="small" icon="el-icon-star-off" @click="paraEdit(scope.row)">配置参数</el-button>
            </el-row>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :visible.sync="isShowDialog" :title="dialogText[dialogStatus]">
      <el-form ref="dataForm" :rules="rules" :model="tempdata" class="dialogform form2col" label-width="90px">
        <el-form-item label="设备名称" prop="name">
          <el-input v-model="pcname" placeholder="设备名称" maxlength="30"/>
        </el-form-item>
        <el-form-item label="设备编号" prop="deviceNo">
          <el-input v-model="pcdeviceNo" placeholder="设备编号" type="text" auto-complete="off" maxlength="40"/>
        </el-form-item>
...
<script>
import { deviceManufacturer, pcList as fetchList, devicePcIsRepeat, deviceList as senorList, pcAdd as addRow, pcDelete as delRow, pcUpdate as updateRow, pcpadParaConfig as updatePara, pcStaticsInfo } from '@/api/device'
...
export default {
  ...
  data() {
    const chkPcMacRepeat = (rule, value, callback) => {
      devicePcIsRepeat({ mac: value, id: this.tempdata.id }).then(res => {
        console.log(res, 'res')
    ...
    return {
      ...
      pcStaticsTable: [
        { prop: 'deviceTitle', label: '主机总数', fixed: false, width: '' },
      ...
      rules: {
        ...
        deviceNum: [{ required: true, message: '必填信息', trigger: 'blur' },
          { validator: chkPcMacRepeat, trigger: 'blur' },
          { pattern: /^([a-f0-9]{2}:){5}[a-f0-9]{2}$/, message: '请输入正确的Mac地址' }]
      },
      paraRules: {
        dealerId: [{ required: true, message: '必填信息', trigger: 'blur' }, { pattern: /^[0-9]+(.[0-9]{2})?$/, message: '请输入正确的数值' }]
      }
    }
  },
  computed: {
  ...
  watch: {
    'orgnode.code': {
      handler(newval, oldval) {
        ...
        if (this.isactive && this.orgnode.code) { this.getList(); this.getSenorList(); this.getPcStaticsInfo() } else { this.needfresh = true }
      },
      immediate: true
    },
    'isactive': {
      handler(newval, oldval) {
        ...
        if (newval && this.orgnode.code) { this.getList(); this.getSenorList(); this.getPcStaticsInfo(); this.needfresh = false }
      }
    }
  },
  created() {
  ...
  methods: {
    // chkPcMacRepeat() {
    //   devicePcIsRepeat({ mac: this.tempdata.deviceNum, id: this.tempdata.id }).then(res => {
    //     console.log(res, 'res')
    //     if (res.data.result === false) {
    //       this.$message({
    //         showClose: true,
    ...
    getPcStaticsInfo() {
      pcStaticsInfo({ orgId: this.orgnode.code }).then(res => {
        this.pcStatisticsData = [res.data]
      })
    },
    getSenorList() {

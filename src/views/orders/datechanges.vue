    ...
    <el-table
      ...
      <el-table-column align="center" prop="created_at" label="操作" width="220">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status==='3'" type="primary" size="small" @click="issueHandle(scope.row)">出票</el-button>
                ...
                <el-table-column :fixed="item.fixed" :key="item.id" :prop="item.prop" :label="item.label" :min-width="item.width" align="center">
                  <template slot-scope="scope">
                    <template v-if="item.prop == 'photo' && scope.row[item.prop]">
                      <div class="photobox">
                        <img v-for="(itemp,pi) in JSON.parse(scope.row[item.prop]) " :src="'http://wuzhou.libokai.cn/'+itemp" :key="'p'+pi" alt="">
                      </div>
                    </template>
                    <template v-else>
                      <span>{{ scope.row[item.prop] | showstr(item.prop) }}</span>
          ...
          <el-form ref="scheduleform" :model="scheduleform" :rules="schrules" class="col2form scheduleform" label-width="80px">
            <template v-for="(schitem,i) in scheduleform.ticket_info" >
              <section :key="'sch'+i">
                ...
                <el-button v-if="i== scheduleform.ticket_info.length-1" type="success" class="btnadd" icon="el-icon-plus" circle/>
              </section>
...
<script>
import { getList as fetchList, updateRowSchdule, issueOrder, getRowSchdule, claimOrder, transferOrder } from '@/api/orders'
...
export default {
  ...
  data() {
    return {
      colTable: [
        ...
        { prop: 'start_date', label: '日期', fixed: false, width: '' },
        { prop: 'contacts', label: '人数', fixed: false, width: '' },
  ...
  methods: {
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        this.list = response.data.map(v => {
          if (v.ticket_info) {
            v.ticket_info = JSON.parse(v.ticket_info.replace(/&quot;/g, '"'))
          } else {
            v.ticket_info = [{ start_city: '' }]
          }
          return v
        })
    ...
    handleEdit(row) {
      ...
      this.scheduleform = Object.assign({}, row)
    ...
    issueHandle(row) {
      this.$confirm('确定已出票吗？', '提示', { type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消' }).then(
        () => {
          issueOrder(row).then((res) => {
            if (res.status === 1) {
              row.status = '4'
              this.$notify({
                title: '成功',
    ...
    setSchedule() {
      this.$refs['scheduleform'].validate((valid) => {
        console.log(valid)
        console.log(this.scheduleform.total)
        if (valid) {
          ...
          updateRowSchdule(tmpform).then((res) => {
            if (res.status === 1) {
              ...
              this.list.forEach(v => { if (v.id === this.scheduleform.id) { v.status = '2' } })
              this.$notify({
                title: '成功',
                message: '发送行程成功',
                type: 'success',
